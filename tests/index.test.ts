...
describe("generateThemePreset", () => {
  ...
  it("should convert theme tokens to Tailwind CSS variables", () => {
    ...
    if (hasFontWeight) {
      expect(result).toMatch(/--font-weight-[\w-]+:\s*var\(--fontWeight[\w]+\)/);
    }
  ...
  it("should exclude spacing and duration tokens", () => {
    ...
    const spacingLines = lines.filter((line) =>
      line.includes("spacing") && !line.includes("--*: initial")
    );
    const durationLines = lines.filter((line) =>
      line.includes("duration") && !line.includes("--*: initial")
    );
  ...
  it("should place common tokens in :root", () => {
    ...
    if (commonKeys.length > 0) {
      ...
      expect(result).toMatch(
        new RegExp(`:\\s*root\\s*\\{[\\s\\S]*${cssVarName}:\\s*${String(value).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`)
      );
    }
  });
